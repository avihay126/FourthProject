...
public class RealEstate {
    ...
    public final int
            SIZE_ARRAY_ADDRESS = 10, SIZE_PHONE_NUMBER_LENGTH = 10;
    public final int ONE_LOCATION = 1, MAX_POST_FOR_BROKER = 10, MAX_POST_FOR_REGULAR = 3;
    ...
    private boolean isProperPhoneNumber(String phoneNumber) {
        ...
        if (phoneNumber.length() == SIZE_PHONE_NUMBER_LENGTH && phoneNumber.charAt(0) == '0'
                && phoneNumber.charAt(ONE_LOCATION) == '5') {
    ...
    private boolean isCanPost(User user) {
        ...
        if (user.isMediatorOrRegular() && counter == MAX_POST_FOR_BROKER) {
            canPost = false;
        } else if (!user.isMediatorOrRegular() && counter >= MAX_POST_FOR_REGULAR) {
            canPost = false;
        }
        return canPost;
    }
